"""
Example: Annual sunlight analysis using Ladybug for all daylight hours in a year
"""
import sys
...
if __name__ == "__main__":
    print("="*70)
    print("ANNUAL SUNLIGHT ANALYSIS - Vertex-Based (No Backface Culling)")
    print("="*70)
    ...
    print("Using VERTEX POSITIONS and VERTEX NORMALS (No Backface Culling)")
    print("="*70)
    ...
    try:
        ...
        print("\nRunning annual sunlight analysis on VERTICES...")
        print("  (This may take a while due to large number of sun positions)")
        print("  BACKFACE CULLING: Disabled (analyzes all vertices regardless of orientation)")
        start_time = time.time()
        
        hitcounts_annual = direct_sunlight.raycast_directional_batch(
            vertices, faces, vertices, vertex_normals,
            light_directions_annual, offset_distance=0.001,
            use_backface_culling=False  # Explicitly disable for vertex-based analysis
        )
        
        elapsed = time.time() - start_time
        print(f"  Completed in {elapsed:.2f} seconds ({elapsed*1000:.1f} ms)")
        print(f"  Processed {len(vertices) * len(light_directions_annual):,} rays")
        print(f"  Performance: {(len(vertices) * len(light_directions_annual)) / elapsed:,.0f} rays/second")
        ...
        print(f"\nAnnual sunlight hours per VERTEX:")
        ...
        print("\nCreating 3D scene for annual data (vertex-based)...")
        scene_annual = visualization.create_scene_with_mesh_and_wireframe(
            vertices=vertices,
            faces=faces,
            vertex_colors=vertex_colors_annual,
        ...
        print("\nShowing 3D visualization for ANNUAL VERTEX analysis...")
        print("Color legend:")
        print("  Red/Orange/Yellow: Maximum annual sunlight exposure")
        print("  Blue/Dark Blue: Minimum annual sunlight exposure (occluded)")
        print("  Yellow arrows: Subsampled sun direction vectors across the year")
        print("\nNote: Vertex-based analysis (no backface culling)")
